    ...
    if 'master_data' in st.session_state:
        ...
        st.components.v1.html(f"""
            <div style="background:#111; padding:30px; border-radius:15px; text-align:center; color:white; font-family:sans-serif;">
                ...
                <div id="scriptArea" style="background:#1a1a1a; padding:25px; border-radius:10px; line-height:2.8;">
                    {subtitle_html}
                </div>
                <audio id="player" src="data:audio/wav;base64,{st.session_state.audio_b64}"></audio>
                <button onclick="playDemo()" style="margin-top:25px; padding:15px 40px; border-radius:30px; background:#27ae60; color:white; border:none; font-weight:bold; cursor:pointer;">
                    🔁 Listen & Guide Start
                </button>
            </div>
            <script>
